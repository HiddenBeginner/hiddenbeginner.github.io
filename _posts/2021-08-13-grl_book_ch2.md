...
![graphlets](https://raw.githubusercontent.com/HiddenBeginner/hiddenbeginner.github.io/master/static/img/_posts/2021-08-13-grl_book_ch2/graphlets.png){: width="400" height="400"){: .center}
<center>[노드가 2개부터 5개까지의 가능한 모든 graphlets을 보여준다. 출처: <a href="#ref4">논문 [4]</a>]</center>

<br>

노드의 feature 벡터는 해당 노드에서 만들 수 있는 `graphlets`의 개수를 종류별로 세서 히스토그램 벡터를 만든다. 이때, 같은 `graphlet`이더라도 어떤 노드를 $u$와 대응하여 볼 것이냐에 따라서 만들어지는 부분 그래프가 달라질 것이다. 예를 들어, $G_1$의 1번을 노드 $u$에 대응했을 때 만들어지는 부분 그래프와 2번을 대응했을 때 만들어지는 부분 그래프가 달라질 것이다. 따라서 노드 임베딩을 만들 경우 한 노드에서 고려해야 할 경우의 수가 총 73가지이기 때문에 73차원 벡터가 만들어진다.<br><br>
