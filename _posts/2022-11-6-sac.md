...
원래 우리가 알고 있는 상태가치함수의 성질에서 확률 분포 $\pi(\cdot \| s)$의 엔트로피만 추가된 형태이다. 마이너스 붙은 것까지 엔트로피이므로 엔트로피가 클수록 가치함수도 커진다. 상태 $s$의 가치는 행동들의 평균 가치에 엔트로피를 보너스로 얹어준 것이다. 다양한 행동을 많이 할수록 더욱 가치가 높은 상태이다. 가치함수의 정의를 바꿔줬다는 점이 정말 획기적인 것 같다. 
